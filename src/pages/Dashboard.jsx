import { ArrowBigRightDashIcon, ArrowRightIcon, KeyboardIcon, PlusIcon, ShieldCheckIcon } from 'lucide-react'
          ...
          <div className='space-y-3' >
            <div className=''>
              ...
              <p className='text-slate-700 text-base sm:text-lg max-w-xl leading-relaxed'>
                Connect, collaborate and celebrate from everywhere with ultra-low latency video, screen-sharing and real-time chat.
              </p>
              <div className='flex flex-col sm:flex-row items-stretch sm:items-center gap-4 pt-2'>
                <button onClick={handleCreateMeeting}
                  disabled={isCreating}
                  className='bg-primary hover:bg-primary-hover text-white font-medium px-6 py-3.5
                 rounded-full shadow-md shadow-primary/20 flex items-center justify-center gap-2.5
                ...
                <form onSubmit={handleJoinMeeting} className='flex flex-1 items-center gap-2'>
                  <div className='relative flex-1'>
                    ...
                    <input type="text" value={joinId} onChange={(e) => setJoinId(e.target.value)}
                      placeholder="Enter Meeting Code (e.g. abc-deg-ghi)"
                  ...
                  <button type='submit' disabled={!joinId.trim()} className='bg-slate-900 hover:bg-slate-800 
                  disabled:opacity-40 disabled:hover:bg-slate-900 text-white font-medium px-6 py-3.5 rounded-full 
                  transition-all flex items-center justify-center cursor-pointer shadow-xs'>
                    <span>Join</span><ArrowRightIcon className='w-4 h-4 wl-1.5' /></button>
                </form>
              </div>
            </div>
          </div>
