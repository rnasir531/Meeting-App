...
const VideoTile = ({ stream, name, isLocal = false, audioEnabled = true, videoEnabled = true }) => {
  ...
  // Handle dynamic audio mute state
  useEffect(() => {
    if (videoRef.current) {
      videoRef.current.muted = isLocal || !audioEnabled;
    }
  }, [isLocal, audioEnabled]);

  return (
    <div className='relative w-full h-full min-h-48 bg-slate-900 rounded-2xl overflow-hidden border border-slate-800 shadow-md flex items-center justify-center group'>
      ...
      <video 
        ...
        className={`w-full h-full object-cover transition-opacity duration-300 ${
          videoEnabled ? "opacity-100" : "opacity-0 pointer-events-none absolute"
        } ${isLocal ? "-scale-x-100" : ""}`} 
      />
      ...
      {!videoEnabled && (
        <div className='flex flex-col items-center justify-center space-y-3 z-10'>
          ...
          <span className='text-xs font-semibold px-3 py-1 rounded-full bg-slate-800/90 text-slate-300 border border-slate-700/60 flex items-center gap-1.5 shadow-sm'>
