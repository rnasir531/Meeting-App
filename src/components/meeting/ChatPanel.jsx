...
const ChatPanel = ({ isOpen, onClose, messages, onSendMessage, currentUser }) => {
    ...
    return (
        <aside className='w-full sm:w-80 h-full bg-white border-slate-200 flex 
        flex-col z-30 shadow-2xl animate-in slide-in-from-right duration-200'>
            ...
            <div className='flex-1 p-4 overflow-y-auto space-y-4' >
                ...
                {messages.length === 0 ? (
                    <div className='h-full flex flex-col items-center justify-center text-center text-slate-400 text-sm'>
                        <p>No Message yet.</p>
                        <p className='text-xs mt-1 text-slate-400'>Send a message to start chatting with partcipants!</p>
                    </div>
                ) : (
                        ...
                        return (
                            <div key={msg.id || index} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                                <div className='flex items-center gap-2 mb-1'>
                                    <span className='text-xs fot-semibold text-slate-500'>{isMe ? "You" : msg.senderName}</span>
                                    <span className='text-[10px] text-slate-400'>{msg.time}</span>
                                </div>
                                <div className={`px-3.5 py-2.5 rounded-2xl max-w-[85%] text-sm leading-relaxed shadow-xs ${isMe
                                    ? "bg-primary rounded-tr-none font-medium"
            ...
            <form onSubmit={handleSubmit} className='p-3 border-t border-slate-200 bg-slate-50 flex items-center gap-2'>
                ...
                <button
                    ...
                    disabled:cursor-not-allowed transition-all cursor-pointer shadow-sm flex items-center justify-center">
