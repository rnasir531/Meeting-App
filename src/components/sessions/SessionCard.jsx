        ...
        justify-between space-y-5 border border-slate-100/60 shadow-xs' >
            ...
            <div className='flex items-center justify-between gap-3 pt-2' >
                <button onClick={() => onOpenDetails(session.id)}
                    className='w-full bg-slate-400/10 hover:bg-slate-400/20 text-slate-800 font-medium py-2.5 px-4 text-xs 
                            transition-all cursor-pointer text-center'>
                    View Details
                </button>
                {!isEnded && (
                    <button onClick={() => onRejoin(session.meetingId)}
                        className='w-full bg-primary hover:bg-primary-hover text-white font-medium py-2.5 px-4 rounded-full 
                            text-xs transition-all shadow-xs cursor-pointer text-center'
                    >
                        Re-join
                    </button>
                )}
